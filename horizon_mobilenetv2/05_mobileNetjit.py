...
def main():
    ...
    torch.jit.save(script_model, os.path.join(model_pathmodel_path+"/jit/", "int_model.pt"))
    ...
    ret = compile_model(
        script_model,
        [example_input],
        hbm=os.path.join(model_path+"/jit/", "model.hbm"),
    ...
    ret = perf_model(
        script_model,
        [example_input],
        out_dir=os.path.join(model_path+"/jit/", "perf_out"),
        input_source="pyramid",
        opt=compile_opt,
        layer_details=True,
    )
